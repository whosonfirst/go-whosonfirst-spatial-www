...
debug-protomaps:
	go run -mod vendor cmd/server/main.go -map-provider protomaps -protomaps-serve-tiles -protomaps-bucket-uri file://$(CWD)/fixtures -protomaps-database sfo -protomaps-paint-rules-uri file://$(CWD)/fixtures/protomaps.rules.paint.js -protomaps-label-rules-uri file://$(CWD)/fixtures/protomaps.rules.label.js
