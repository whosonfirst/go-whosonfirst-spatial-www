...
func AppendWWWFlags(fs *flag.FlagSet) error {
	...
	fs.Bool("enable-data", false, "Enable the /data endpoint to return WOF records (as GeoJSON)")	
	fs.String("data-reader-uri", "", "A valid whosonfirst/go-reader.Reader URI. Required if the -enable-geojson or -enable-www flags are set.")
